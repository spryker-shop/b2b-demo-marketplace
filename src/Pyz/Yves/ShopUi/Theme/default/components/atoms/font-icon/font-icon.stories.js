...
const docs = componentDocs({
    ...
    data: [
        { prop: 'name', type: 'string', default: 'required', desc: 'Material Symbols icon name' },
    ],
    attributes: [
...
export default { title: 'Atoms/Font Icon' };

const iconNames = [
    'home', 'person', 'search', 'settings', 'close',
    'check', 'add', 'edit', 'delete', 'arrow_forward',
    'shopping_cart', 'favorite', 'visibility', 'info', 'warning',
];

export const Overview = {
    render: () => {
        return (
            section('Outlined icons',
                ...
                iconNames.map((name) =>
                    ...
                    '<span style="font-size: 11px; color: #888;">' + name + '</span></div>'
                ).join('') +
                '</div>'
            ) +
            section('Filled',
