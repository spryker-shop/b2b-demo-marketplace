...
export const Overview: StoryObj = {
    render: () => {
        ...
        return (
            section('Default', frame('10rem', renderAtom('thumbnail', { attributes: { src: img, alt: 'Default' } }))) +
            section(
                'Smaller',
                frame(
                    '3rem',
                    renderAtom('thumbnail', { modifiers: ['smaller'], attributes: { src: img, alt: 'Smaller' } }),
                ),
            ) +
            section(
                'Small',
                frame(
                    '5rem',
                    renderAtom('thumbnail', { modifiers: ['small'], attributes: { src: img, alt: 'Small' } }),
                ),
            ) +
            section(
                'Expand',
