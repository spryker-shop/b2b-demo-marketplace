...
namespace Pyz\Zed\ShopConfiguration\Communication\Plugin\Store;
...
class CreateStoreDomainStorePostCreate extends AbstractPlugin implements StorePostCreatePluginInterface
{
    ...
    public function execute(StoreTransfer $storeTransfer): StoreResponseTransfer
    {
        ...
        if (!$storeTransfer->getIdTenant()) {
            return $storeResponseTransfer;
        }

        $tenantTransfer = (new \Pyz\Zed\TenantOnboarding\Business\TenantOnboardingFacade())
            ->findTenantByIdentifier($storeTransfer->getIdTenant());
