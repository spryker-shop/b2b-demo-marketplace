...
- **`architecture/preview-standalone.html`** — the **handoff / offline** view, compiled **on demand at
  the end** by the **pure-Bash builder** (base64 JSON island; no Python) that bakes every section +
...
**Both files share ONE render core** — do NOT hand-write them or maintain two renderers (they drift:
missing ADRs, different ordering). Generate BOTH with `references/preview-assets/build-previews.sh`,
which splices the shared `head.html` + shared `render-core.js` + a one-line loader (fetch vs island —
the only difference) and **asserts via `shasum` that the render core is byte-identical in both**,
failing if they ever diverge:
```bash
ARCH="<ABS>/architecture" ASSETS="<SKILL>/references/preview-assets" \
  bash "<SKILL>/references/preview-assets/build-previews.sh"
```

Why two files at all: browsers block `fetch()` on `file://`, so no single file can both open on
`file://` and read the live folder — live-read needs a server, double-click needs baked content. In Gated mode ask which the user wants; in Autonomous
...
| Preview / PDF export | Both files generated by `references/preview-assets/build-previews.sh` (shared `head.html`+`render-core.js`, `shasum`-asserted identical core) per [preview.md](references/preview.md): `preview.html` (dev — served, auto-discovers via HEAD probes + SD/ADR READMEs, any static server) and `preview-standalone.html` (handoff — bakes all content into one `file://`-openable file; `Cmd/Ctrl+P` Save-as-PDF or headless-Chrome `--print-to-pdf`) |
