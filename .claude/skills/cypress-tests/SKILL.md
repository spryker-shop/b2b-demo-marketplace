...
description: >
  ...
  project-owned Cypress baseline under `tests/e2e/` (vendored from
...
This project's Cypress suite lives entirely under `tests/e2e/` as a self-contained Node
project — its own `package.json`, not a workspace of the root repo and not a Composer
dependency. It replaces Spryker's internal demo-shop Cypress/Robot Framework packages, which
are for Spryker's own core-feature testing and aren't guaranteed to work on a customized
project.

## Step 1 — Know the conventions before writing anything

- **Directory layout**: specs under `tests/e2e/cypress/e2e/<app>/<feature>/`, where `<app>` is
  `storefront`, `backoffice`, `merchant-portal`, or `glue`. Fixtures under
  `tests/e2e/cypress/fixtures/`. Reusable logic under `tests/e2e/cypress/support/`
  (`page-objects/`, `scenarios/`, `cy-commands/`, `glue-endpoints/`, `api-helper/`).
...
- **Page Object pattern**: specs never call `cy.get()`/`cy.visit()` directly — they only
  ...
  before writing a new one — check `tests/e2e/cypress/support/page-objects/` and
  `.../scenarios/` first.
- **TypeScript path aliases**: import fixtures via `@fixtures/*` and support code via
  `@support/*` (see `tests/e2e/tsconfig.json`), not relative `../../..` paths.
...
1. Identify the existing page objects/scenarios that already cover the flow (search
   `tests/e2e/cypress/support/page-objects/` and `.../scenarios/`) — extend them rather than
...
cp .env.dist .env           # first time — fill in any secrets, real per-environment values live in .envs/
...
(default `local`); each maps to a file in `tests/e2e/.envs/`. CI runs the same
...
- `tests/e2e/README.md` — this project's own setup notes.
