...
- [x] deploy.spryker-icpplus.yml audited vs sibling deploy files — entry-points, env keys, and `image.php.ini` value diff (Step 6d)
...
- **A demo-only FILE can't receive master's fleet-wide edits — divergence also runs backwards.** `deploy.spryker-icpplus.yml` exists only on `master-demo`, so when master tunes a runtime block across the deploy files it *can* see (`icp`, `scos`, `sns`, `aws-env-template`), icpplus silently keeps the old values. Nothing conflicts, no key is *missing* (the keys are present with stale *values*), and no merged code "requires" anything — so Step 6d's checks 1–3 all report clean. Only the **value-level `image.php.ini` diff (check 4)** finds it. Real case: CC-39382 enabled opcache everywhere else, leaving icpplus the lone cloud env on `opcache.enable: 0`. When copying the fix, **diff more than one sibling** — they disagree on purpose (`memory_consumption` is 256 on `icp`, 128 on the rest; `aws-env-template` is the canonical default).
